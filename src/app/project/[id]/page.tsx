...
export default async function ProjectDetails({ params }: { params: { id: string } }) {
  ...
  let captador = null;
  if (project?.captador_id) {
    const { data: captadorData } = await supabase
      .from("team_members")
      .select("*")
      .eq("id", project.captador_id)
      .single();
          ...
          {captador && (
            <div className="mt-16 pt-12 border-t border-neutral-100">
              <div className="max-w-md mx-auto bg-white/50 backdrop-blur-sm rounded-3xl p-6 border border-neutral-200/60 shadow-sm flex items-center gap-6 group hover:border-[#2C2C2C]/20 transition-all duration-500">
                ...
                <div>
                  <span className="inline-block px-2 py-0.5 rounded-full bg-neutral-100 text-[10px] font-bold text-neutral-500 uppercase tracking-widest mb-2 group-hover:bg-[#FFB800]/10 group-hover:text-[#FFB800] transition-colors">Captação Especializada</span>
